...
export const CirculationCenter = (props) => {
    ...
    const createCurrency = (form, type, amount) => {
        try {
            const amt = amount ? amount : 1;
            if (form && type) {
                ...
                if (form == 0) {
                    createCall = Coin(id=id, type=type, value=value);
                } else if (form = 1) {
                    createCall = Bill(id=id, type=type, value=value);
                }
                
                if (createCall) {
                    let i = 0;
                    while (i < amt) {
                        currencystack.push(createCall);     console.log(currencystack);
                        i++;
                    }
                }
            }
